...
def load_and_process_bjd():
    try:
        # 파일 읽기 (한글 깨짐 방지를 위해 cp949 또는 utf-8-sig 사용)
        df = pd.read_csv("법정동코드.csv", encoding="cp949")
    except UnicodeDecodeError:
        df = pd.read_csv("법정동코드.csv", encoding="utf-8-sig")
        
    # 현재 존재하는 지역만 남기기 (폐지된 지역 제외)
    df = df[df["폐지여부"] == "존재"]
    
    # 법정동 코드를 다루기 쉽게 문자열로 변환하고 10자리 맞추기
    ...
    def split_bjd_name(name):
        ...
        if len(parts) == 2:
            ...
            # 수원시 영통구, 창원시 마산합포구 같은 4단어 이상 지역 처리
    ...
    # 새로운 컬럼으로 추가
    df[["시도", "시군구", "읍면동"]] = df["법정동명"].apply(split_bjd_name)
    
    # 동 정보가 없는 시군구 자체 행은 제외 (실제 매매 조회를 위해)
    df = df[df["읍면동"] != ""]
    return df
...
with col3:
    ...
    # API에 던질 5자리 시군구 코드 (법정동코드의 앞 5자리)
    target_row = dong_filtered[dong_filtered["읍면동"] == selected_dong].iloc[0]
...
if st.button("실거래가 데이터 조회하기", type="primary"):
    ...
    params = {
        'serviceKey': API_KEY,
        'pageNo': '1',
        'numOfRows': '2000', # 해당 구 전체를 가져와야 하므로 넉넉하게 설정
        'LAWD_CD': lawd_cd,
        'DEAL_YMD': deal_ymd
    }
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if len(items) > 0:
                ...
                # 전체 구 데이터를 데이터프레임으로 변환
                df_all = pd.DataFrame(raw_data)
                
                # [💡 핵심 단계] 사용자가 선택한 '동' 이름과 정확히 일치하는 행만 필터링!
